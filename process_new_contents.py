...
async def main():
    ...
    # Step 1: Extract perceptions for unprocessed contents
    print(f"\n[1/2] 미처리 {unprocessed}개 분석 (GPT-5)")
    print("-" * 60)
    ...
    perception_ids = []
    for i, content in enumerate(unprocessed_contents, 1):
        print(f"\r[{i}/{len(unprocessed_contents)}] {content['title'][:50]}...", end='', flush=True)
        try:
